...
- Analyze 3-4 major competitors in AI-powered employee onboarding tools (Deel, Rippling, BambooHR, Leena AI)
- Summarize their key features, strengths, and weaknesses
- Identify current market trends in AI-powered onboarding, HR automation, and employee enablement
...
After presenting your blueprint, invite the CostAnalyst to estimate implementation costs and tradeoffs.
Keep your response focused and under 400 words.""",
            llm_config=self.llm_config,
            description="A product designer who creates feature blueprints and user journeys based on identified market opportunities.",
        )

        # Cost Agent - estimates implementation cost before executive review
        self.cost_agent = autogen.AssistantAgent(
            name="CostAnalyst",
            system_message="""You are a financial analyst specializing in SaaS product planning.
Your role in this group discussion is to EVALUATE the proposed product from a cost, timeline, and ROI perspective.

Your responsibilities:
- Review the BlueprintAgent's proposed MVP features and estimate relative implementation effort
- Highlight the 3 most expensive or risky items and explain why
- Provide a simple phased timeline (MVP, V1, V2)
- Rank the proposed features by cost-benefit tradeoff

Reference specific features from the BlueprintAgent and opportunities from earlier analysis.
After presenting your cost analysis, invite the ReviewerAgent to provide final recommendations.
Keep your response focused and under 400 words.""",
            llm_config=self.llm_config,
            description="A financial analyst who estimates implementation cost, timeline, and ROI tradeoffs for the proposed product.",
        )
...
Your responsibilities:
- When the BlueprintAgent and CostAnalyst present their recommendations, evaluate overall feasibility and market fit
- Provide 3-4 strategic recommendations for launch success
- Suggest a phased implementation approach (MVP → V1 → V2)
- Identify key risks and mitigation strategies

Reference specific features from the BlueprintAgent, cost tradeoffs from the CostAnalyst, and opportunities from earlier discussion.
After your review, conclude the discussion by ending your message with the word TERMINATE.""",
    ...
    def run(self):
        ...
        print("\n" + "=" * 80)
        print("AUTOGEN GROUPCHAT - AI PLATFORM PRODUCT PLANNING")
        print("=" * 80)
...
4. CostAnalyst: Estimate implementation cost, timeline, and ROI tradeoffs
...
ResearchAgent, please begin with your market analysis."""
        ...
        chat_result = self.user_proxy.initiate_chat(
            ...
            summary_args={
                "summary_prompt": "Summarize the complete product plan developed through this multi-agent discussion. Include: key market findings, identified opportunities, proposed features, and strategic recommendations for the AI-powered employee onboarding platform."
            },
        )
